# VSL de FAP — modelo "delegar las ventas" (adaptada del VSL de Sandler)

> **Generada por el cerebro FAP** · skill `fap-vsl` (+ `fap` fuente de verdad). Ruta A: Ads → **VSL** →
> Diagnóstico. Guion hablado, Jorge a cámara, tono conversacional.
>
> **Conserva del original:** estructura y recursos retóricos — hook de pérdida (loss-aversion) →
> callout de ICP + agitación → reencuadre del sistema → "¿cómo lo haremos?" con **palancas numeradas** +
> testimonio por palanca → el "secreto" del playbook para delegar → reversión de riesgo → autoridad →
> **"tres razones"** → cierre anti-presión + CTA → casos de éxito post-agenda → cierre cálido.
>
> **Candado #1 — qué se transformó y por qué:**
> - **✅ GARANTÍA REAL DE FAP (política oficial de Jorge).** El original promete "30% más o te
>   devolvemos tu dinero". Se reemplaza por la garantía real y citable: *"si no duplicas la rentabilidad
>   por vendedor que hoy tienes, seguimos trabajando contigo hasta conseguirlo"* — remedio = trabajar
>   hasta lograrlo, **nunca devolución de dinero**; condicionada a "para quien implementa". Va en el
>   **hook** y reforzada en la reversión. Respaldo operativo (90 días / 80% de procesos → extensión) y
>   detalle en `contexto/jorge_decisions.md` §4.
> - **✅ Mecanismo = los 6 pilares** (no "4 palancas"): metodología · habilitación · prospección con IA ·
>   supervisión · comisiones · contratación, todos alineados al proceso de compra del cliente.
> - **Autoridad Sandler → PBS.** Fuera Sandler / Teresa Prieto / Pinnacle Award / Selling Power /
>   "método Sandler" / podcast Máquina de Ventas. Entra Jorge Conde + cifras sourced.
> - **Testimonios del original NO se usan** (Mariana Villegas, Santiago Cala, Freddy Gallego, Natalya
>   Caro): no están en `contexto/`. Se usan los **2 casos reales** de FAP; los slots extra van `[FALTA]`.
> - **ICP:** el original usa "$500K"; FAP usa **≥$1M/año + ≥2 vendedores** (piso duro <$500K).
> - **Cifras/claims sin fuente fuera** (ciclo "a la mitad", "10%–300% anual", stat "CRM +29%"):
>   sustituidos por sourced (−25% ciclo, +$150M, SROI >5X, casos).
> - **CTA:** "llamada con mi equipo" → **diagnóstico**. Sin precio.

---

## Guion

¿Cuánto dinero estás dejando de ganar porque cada cierre importante sigue dependiendo de ti? Te
acompañamos, paso a paso, a instalar el sistema para que empieces a delegar las ventas de tu empresa; y
si no duplicas la rentabilidad por vendedor que hoy tienes, seguimos trabajando contigo hasta
conseguirlo.

Si diriges una empresa que factura un millón de dólares o más, tienes un equipo de ventas y aun así
eres tú el principal responsable de que los negocios de alto valor se cierren, sabes que necesitas
crecer, vender más y abrir nuevos mercados… pero estás atrapado en ciclos de venta eternos, propuestas
que nadie responde y decenas de reuniones que no terminan en venta.

Déjame mostrarte el sistema que ya instalamos en empresas como la tuya: el que transforma equipos
empíricos, vendedores inseguros y cierres impredecibles en un proceso comercial con estructura, control
y resultados que se repiten semana a semana.

Es otra forma de vender: sin perseguir a nadie, sin forzar cierres y sin que tengas que estar encima de
todo. Donde el ciclo de venta se acorta, y cada vendedor sabe exactamente qué decir, qué preguntar y
cómo avanzar desde el primer contacto hasta el cierre.

¿Y cómo lo hacemos? Con el mismo sistema que aplicamos desde PYMES hasta empresas Fortune 500, y que le
ha sumado a nuestros clientes más de 150 millones de dólares en nueva facturación. Lo adaptamos a tu
empresa y a tu equipo, y todo gira alrededor de una sola cosa: cómo compra de verdad tu cliente. Lo
instalamos con nuestro sistema de 6 pilares.

**Uno, la metodología de ventas:** rediseñamos a quién le vendes y cómo le argumentas valor, para que
dejes de competir por precio y dejes de ser uno más del montón.

**Dos, la habilitación comercial:** los guiones, los playbooks y el manejo de objeciones para que tu
equipo cierre sin incomodar, sin dar descuentos y sin negociaciones eternas. Porque la mayoría de los
vendedores no se paraliza por falta de capacidad, sino por falta de entrenamiento y de herramientas.

**Tres, la prospección a gran escala con inteligencia artificial:** para llenar tu pipeline con
compradores que sí califican, y decirle adiós a las citas innecesarias y a los seguimientos eternos con
quien nunca te iba a comprar. Mira lo que pasó con una empresa de software B2B que factura más de 22
millones al año: sus vendedores lograban apenas 1 reunión por semana, y en 4 meses cada uno pasó a 4,
con la tasa de cierre subiendo del 10% al 19% y el cumplimiento de cuota del 45% al 89%. Los mismos
vendedores. Otro sistema.

**Cuatro, la supervisión comercial:** nos convertimos en ese gerente de ventas que te hace rendir
números, llenar el CRM y ejecutar, y que detecta una caída antes de que golpee tu facturación. Ese peso
de cargar tú solo con las ventas lo cargamos contigo — no vas a estar solo. Una multinacional industrial
en más de 100 países la instaló y, en 120 días, generó 16.7 millones de dólares de pipeline nuevo por
trimestre. Su vicepresidente de ventas en México lo dijo así: pasaron de depender de la cartera de
siempre a un sistema con foco, con supervisión predictiva y con capacidad real de crecer cada trimestre.

**Cinco, el sistema de comisiones:** para que todo tu equipo rema hacia el mismo lado, con incentivos
que empujan la facturación que tú quieres.

**Y seis, el sistema de contratación:** contratar por perfil y no por corazonada. Porque hoy, en
Latinoamérica, de cada 4 vendedores que contratas solo 1 realmente funciona; y no es mala suerte, es que
se contrata por feeling.

Seis pilares, engranados entre sí. Y cuando quedan montados, pasa lo que de verdad quieres: las ventas
dejan de depender de que estés tú en la sala. Ahí empiezas a delegar de verdad y a recuperar tu tiempo —
para pensar nuevos proyectos, para descansar, para tu familia— y dejas de ser esclavo de tu empresa.

¿Y sabes por qué casi todos fracasan al intentar delegar? Porque lo intentan a punta de nómina,
rotación y cero resultados. Déjame contarte un secreto: sin un playbook de ventas, nunca vas a poder
delegar. Ese es el punto que casi nadie te dice.

Y para que entres sin miedo, te doy una garantía que casi nadie en este mercado se atreve a dar: si al
implementar el sistema no duplicas la rentabilidad por vendedor que hoy tienes, seguimos trabajando
contigo, sin costo adicional, hasta que lo consigas. Eso sí, te lo digo de frente: esto funciona para
quien implementa, ejecuta y sigue el paso a paso. Los números que te doy son de clientes que lo
hicieron — y, de hecho, en los dos casos que te conté la rentabilidad por vendedor no se duplicó: se
multiplicó por más de dos y medio.

Ahora, si no nos conoces, me presento en 20 segundos para que sepas quién te va a llevar de la mano
estos meses. Soy Jorge Conde, socio fundador de PIPELINE Business School. Desde 2019 hemos formado a más
de 47 mil profesionales de ventas y a más de 10 mil directivos, y hemos reestructurado directamente más
de 120 empresas B2B que facturan, en promedio, más de 2 millones de dólares al año — algunas de ellas,
Fortune 500. De hecho, muchos de los directivos que entrenamos hoy dirigen las ventas en marcas como
Google, SAP o Cisco. Somos la única aceleradora de ventas que trabaja bajo la alianza de Harvard
Business Publishing, y nos han destacado en Forbes y en el CIO Times. Y todo lo que te enseñamos, ya lo
aplicamos: no es un entrenamiento motivacional ni relleno; son estrategias específicas, al grano y
accionables. Así que si todo esto que ya sabes que funciona te hace sentido, toca el botón de abajo para
agendar tu diagnóstico.

Y si quieres más razones de por qué este sistema funciona tan bien, dame dos minutos y te doy tres.

**Razón uno: evidencia, no motivación.** Todo lo que hacemos se mide. Por eso 6 de cada 10 de nuestros
clientes recuperan más de 5 veces lo que invirtieron, su forecast pasa a acertar un 85% y sus ciclos de
venta se acortan una cuarta parte. No es opinión ni entusiasmo: es ingeniería comercial, respaldada por
la alianza de Harvard y por medios como Forbes y el CIO Times.

**Razón dos: implementamos contigo, no te dejamos solo.** A diferencia de un curso que te abandona,
nosotros nos metemos a tu operación. Acompañamiento uno a uno con ejecutivos que fueron directivos y
dueños de negocio —no teóricos ni novatos—, que revisan tus números y tus acciones de escala contigo,
semana a semana. Cada cosa que te enseñamos ya la aplicamos en el campo de batalla.

**Razón tres: pensado para dueños con poco tiempo.** Por eso te entregamos PIPELINE OS, nuestra
plataforma, con un equipo de agentes de inteligencia artificial trabajando dentro de tu operación:
prospectan por ti, leen tus números y te dicen dónde poner el foco, y hasta te ayudan a contratar y a
supervisar. La tecnología para que vendas más sin trabajar más horas, y vayas siempre un paso adelante
de tu competencia.

Por eso este sistema logra lo que ninguna maestría, especialización o gerencia comercial tradicional
consigue: transformar tu proceso de ventas de raíz.

Si esto te suena y quieres saber más, toca el botón de abajo para agendar tu diagnóstico. Y te lo digo
claro: no tenemos ninguna motivación para venderte si no estamos seguros de que podemos ayudarte, porque
ponemos en juego nuestro tiempo, nuestro nombre y nuestra energía. El primer paso no es una llamada de
ventas: es un diagnóstico, donde miramos tu operación completa, te decimos exactamente dónde se está
frenando tu facturación y te llevas los primeros pasos concretos… trabajes con nosotros o no.

Después de agendar, te voy a compartir dos videos que es importante que veas, y varios casos reales de
clientes para que veas cómo cambió la operación de otros empresarios como tú. `[FALTA: casos de éxito
adicionales con nombre + cifra + plazo, aportados por Jorge — no se inventan.]`

Nos gusta darle nuestra energía a quien de verdad quiere trabajar con nosotros. Si este no es tu
momento, no hay problema; nuestro estilo no es perseguirte. Mi propuesta es simple: agenda la llamada
para conversar, aclarar tus dudas y descubrir juntos si esta es la oportunidad que estabas buscando.
Toca el botón de abajo para agendar.

Charlamos pronto. Por lo pronto, te deseo mucho éxito y mucha facturación predecible.

---

## Checklist de presión (pre-entrega)

- [x] Estructura y recursos retóricos del original replicados (loss-aversion, pilares numerados,
  "secreto" del playbook, "tres razones", garantía, anti-presión, casos post-agenda, cierre cálido).
- [x] **Garantía real de FAP** (instalación 90 días / 80% o extensión gratis) — sourced, no de resultado.
- [x] Mecanismo = **6 pilares** (no 4 palancas), alineados al proceso de compra del cliente.
- [x] Autoridad PBS (sin Sandler). · Cifras **sourced**. · ICP FAP (≥$1M, ≥2 vendedores).
- [x] Testimonios = 2 casos reales; slots extra `[FALTA]`, no inventados.
- [x] **Sin precio.** · CTA único a **diagnóstico**. · "para quien implementa" presente.
- [x] Tono conversacional · problema sistémico · proceso de compra del cliente como eje.

## Pendientes de Jorge (no se inventan)

1. **Testimonios con nombre** para los "casos post-agenda" (Freddy/Natalya del original no son de FAP).
2. Confirmaciones abiertas de la otra VSL: Google/SAP/Cisco (clientes vs. directivos formados) y la
   discrepancia de cifras de autoridad con `contexto/autoridad.md`.
